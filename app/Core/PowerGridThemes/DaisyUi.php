...
namespace App\Core\PowerGridThemes;
...
class DaisyUi extends Tailwind
{
    public function table(): Table
    {
        return Theme::table('min-w-full dark:!bg-primary-800')
                    ->container('card bg-base-100 rounded-lg shadow-lg')
                    ->base('p-3 align-middle inline-block min-w-full w-full sm:px-6 lg:px-8')
                    ->div('rounded-t-lg relative')
                    ->thead('text-black dark:text-gray-200')
                    ->thAction('!font-bold')
                    ->tdAction('')
                    ->tr('hover:bg-base-200/50 ')
                    ->trFilters('shadow-sm dark:bg-pg-primary-800')
    ...
    public function footer(): Footer
    {
        return Theme::footer()
                    ...
                    ->select(
                        'select select-primary select-sm flex sm:text-sm sm:leading-6 w-auto md:mr-5'
                    );
    }
    ...
    public function actions(): Actions
    {
        return Theme::actions()
                    ->headerBtn(
                        'block w-full bg-pg-primary-50 text-pg-primary-700 border border-pg-primary-200 rounded py-2 px-3 leading-tight focus:outline-none focus:bg-white focus:border-pg-primary-600 dark:bg-pg-primary-800 dark:text-pg-primary-200 dark:placeholder-pg-primary-300 dark:border-pg-primary-600'
                    )
    ...
    public function checkbox(): Checkbox
    {
        return Theme::checkbox()
                    ...
                    ->input(
                        'form-checkbox dark:border-dark-600 border-1 dark:bg-dark-800 rounded border-gray-300 bg-white transition duration-100 ease-in-out h-4 w-4 text-primary-500 focus:ring-primary-500 dark:ring-offset-dark-900'
                    );
    }
